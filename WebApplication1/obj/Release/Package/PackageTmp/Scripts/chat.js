...
const loadMsg = (msg) => {
    ...
    if (!localStorage.loggedUser) {
        ...
        HTMLString += ` <div class="likesNum">
                                        <img data-like="${"likeState"}" title="like" data-msgId="${msg.msgId}" onclick="likeMsg('${msg.msgId}')" src=${"./images/likeBlack.png "}/>
                                        <p>${msg.likes == undefined ? 0 : msg.likes.length}</p>
                                    </div>
                                    <div class="dislikesNum">
                                        <img data-dislike="${"dislikeState"}" title="dislike" data-msgId="${msg.msgId}" onclick="dislikeMsg('${msg.msgId}')" src=${"./images/dislikeBlack.png "}/>
...
const loadReplyMsgs = (msg) => {
    ...
    msg.replies.forEach((reply, index) => {
        ...
        picturesRef.once("value", snapshot => {
            ...
            $('#msg-' + msg.msgId + '-reply-' + index + ' .reactions').append(
                `<div class="likesNum">
                    <img data-like="${"likeState"}" title="like" data-replyIndex="${index}" onclick="likeReply('${msg.msgId}','${index}')" src=${"./images/likeBlack.png "}/>
                    <p>${reply.likes == undefined ? 0 : reply.likes.length}</p>
                </div>
                <div class="dislikesNum">
                    <img data-dislike="${"dislikeState"}" title="dislike" data-replyIndex="${index}" onclick="dislikeReply('${msg.msgId}','${index}')" src=${"./images/dislikeBlack.png "}/>
