...
const openAdvanceSearch = () => {
    ...
    loadAlotOfTvShows(1000);
}
// const loadAlotOfTvShows = async (amount) => {
//     let page = 1;
//     while(AdvancedSearchList.length < amount){
...
//         .catch('could not get top tv shows');
//     AdvancedSearchList = AdvancedSearchList.concat(popularTvShows);
//     }
// }
...
const goToTvShowPage = (name) => {
    console.log(AdvancedSearchList);
    if (APISearchList.length == 0 && AdvancedSearchList.length == 0) {
        alert('This name does not exists in the server. Please try again with another name.');
        return;
    }
    else  window.location.href = 'tvshowpage.html?tvshowName='+ name;
}
...
const addMoreTvShows = (div, type) => {
    ...
    $.ajax({
        ...
        addCardsToDiv(div, tvShows.results)
    }).catch((error) => {
        console.log(error);
    })
}
